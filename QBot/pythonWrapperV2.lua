...
function sysCall_init(...)
    ...
    replyPortStr = getFreePortStr(rpcPort)
...
function initPython(prog, rpcPort)
    ...
    if pyth and #pyth > 0 then
        subprocess, controlPort = startPythonClientSubprocess(pyth, rpcPort)
        if controlPort then
...
function startPythonClientSubprocess(pythonExec, rpcPort)
    local subprocess
    local controlPort = getFreePortStr(rpcPort)
