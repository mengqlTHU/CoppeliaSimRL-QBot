...
if __name__ == "__main__":
    # ---------------- Create environment
    # env = QBotEnv(port=23010) # action_type can be set as discrete or continuous
    ...
    # ---------------- Callback functions
    log_dir = "../QBot/saved_models/tmp"
    os.makedirs(log_dir, exist_ok=True)

    env = VecMonitor(env, log_dir)
    # env = Monitor(env, log_dir)

    callback_visdom = VisdomCallback(name='visdom_qbot_rl', check_freq=100, log_dir=log_dir)
    ...
    # model = A2C.load("../QBot/saved_models/tmp/best_model", env=env)
