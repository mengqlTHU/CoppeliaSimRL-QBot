...
if __name__ == "__main__":
    # ---------------- Create environment
    env = QBotEnv_Jump(port=40000) # action_type can be set as discrete or continuous
    ...
    # ---------------- Callback functions
    log_dir = "../QBot/saved_models/tmp/SAC-Tcontrol-Jump-single"
    ...
    # model = SAC(policy='MlpPolicy', env=env, learning_rate=1e-3, verbose=True, train_freq=120, learning_starts=5000, ent_coef='auto_0.5', target_update_interval=120)

    # Option 2: load the model from files (note that the loaded model can be learned again)
    # print("load the model from files")
    # model = SAC.load("../QBot/saved_models/tmp/SAC/best_model_only_pos", env=env)
    ...
    model = SAC.load("../QBot/saved_models/tmp/SAC-Tcontrol-Jump/best_model", env=env)
